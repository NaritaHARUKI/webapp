...
func getSessionUser(r http.Request) User {
    ...
    it, err := memcacheClient.Get(cacheKey)
    if err == nil && it != nil {
        // キャッシュヒット時
        var cachedUser User
        if err := json.Unmarshal(it.Value, &cachedUser); err == nil {
            return cachedUser
        }
    }
...
func makePosts(results []Post, csrfToken string, allComments bool) ([]Post, error) {
    ...
    // 3. 投稿ごとのコメントをまとめて取得（LIMIT制御は難しいのでallComments=false時は全件取るか、後述の工夫を）
    var comments []Comment
    commentQuery := "SELECT * FROM comments WHERE post_id IN (?) ORDER BY created_at DESC"
    commentQuery, cargs, err := sqlx.In(commentQuery, postIDs)
    if err != nil {
        return nil, err
    }
    commentQuery = db.Rebind(commentQuery)
    err = db.Select(&comments, commentQuery, cargs...)
    if err != nil {
        return nil, err
    }

    // 4. コメントに紐づくユーザーIDを集めて一括取得
    ...
    userQuery, uargs, err := sqlx.In(userQuery, userIDs)
    if err != nil {
        return nil, err
    }
    userQuery = db.Rebind(userQuery)
    err = db.Select(&users, userQuery, uargs...)
    if err != nil {
        return nil, err
    }
    ...
    // 5. コメントを投稿ごとに振り分け（and limit 3件ならここで制御）
    ...
    // コメント順を逆にしている部分を再現（元のコードの意図を尊重）
    for pid, clist := range commentsByPost {
        // 逆順
        for i, j := 0, len(clist)-1; i < j; i, j = i+1, j-1 {
            clist[i], clist[j] = clist[j], clist[i]
        }
        if !allComments && len(clist) > 3 {
            clist = clist[:3]
        }
        commentsByPost[pid] = clist
    }

    // 6. 投稿にユーザー情報とコメント情報をセット
    for _, p := range results {
